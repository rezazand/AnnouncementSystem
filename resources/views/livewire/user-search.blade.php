            ...
            <table class="table">
                ...
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>
                            <label title="ویرایش" for="u-{{$user->id}}" class="badge badge-warning"><i class=" ion ion-edit"></i></label>
                            <input type="radio" id="u-{{$user->id}}" onclick="processChange();modalName='editUser';" wire:model="check"
                            ...
                            <div class="input-group mb-3">
                                <select id="department" name="department" class="form-control">
                                    @foreach($departments as $d)
                                        <option @if($d->id == $userE->department_id) selected @endif value="{{$d->id}}">{{$d->label}}</option>
                                    @endforeach
                                </select>
                                <div class="input-group-append">
                            ...
                            <div class="input-group mb-3">
                                <select id="role" name="role" class="form-control">
                                    @foreach($roles as $role)
                                        <option @if($role->id == $userE->role_id) selected @endif value="{{$role->id}}">{{$role->label}}</option>
                                    @endforeach
                                </select>
                                <div class="input-group-append">
